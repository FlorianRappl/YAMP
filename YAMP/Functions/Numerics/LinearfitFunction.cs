...
namespace YAMP
{
    ...
    class LinearfitFunction : ArgumentFunction
    {
        [Description("Linearfit finds the coefficients of a linear combination of n functions, f_j(x(i)) to y(i), in a least squares sense. The result p is a row vector of length n containing the coefficients, i.e. p(1) * f_1 + p(2) * f_2 + p(3) * f_3 ... + p(n) * f_n.")]
        [Example("x=(-2.5:0.1:2.5); linearfit(x, erf(x), [x, x.^3, tanh(x)])", "fits the error function with the function p(1) * x + p(2) * x.^3 + p(3) * tan(x). The result is p = [-0.149151; 0.006249; 1.291217]")]
        public MatrixValue Function(MatrixValue x, MatrixValue y, MatrixValue f)
        {
            ...
            if (m < 2)
                throw new YAMPArgumentInvalidException("Linearfit", "f", 3);
        ...
        [Description("Linearfit finds the coefficients of a linear combination of n functions, f_j(x(i)) to y(i), in a least squares sense. The result is the fit function p(1) * f_1 + p(2) * f_2 + p(3) * f_3 ... + p(n) * f_n.")]
        [Example("x=(-2.5:0.1:2.5); linearfit(x, erf(x), x => [x, x.^3, tanh(x)])", "fits the error function with the function p(1) * x + p(2) * x.^3 + p(3) * tan(x). The result is a function evaluating -0.149151 * x + 0.006249 * x.^3 + 1.291217 * tanh(x)]")]
        public FunctionValue Function(MatrixValue x, MatrixValue y, FunctionValue f)
        {
            var context = ParseContext.Default;//TODO: find correct parse context
